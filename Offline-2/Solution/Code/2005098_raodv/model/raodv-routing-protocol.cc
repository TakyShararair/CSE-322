...
RoutingProtocol::RecvRaodv(Ptr<Socket> socket)
{
    ...
    case RAODVTYPE_R_RREQ: {
        RevRecvRequest(packet, receiver, sender);
        break;
    }
    }
}
...
void
RoutingProtocol::RevSendReply(const RreqHeader& rreqHeader, const RoutingTableEntry& toOrigin)
{
    ...
    // Create a TypeHeader for RREP
    TypeHeader tHeader(RAODVTYPE_R_RREQ);

    // Iterate over all socket addresses to perform broadcast on each interface
    for (auto j = m_socketAddresses.begin(); j != m_socketAddresses.end(); ++j)
    {
        Ptr<Socket> socket = j->first;
        Ipv4InterfaceAddress iface = j->second;

        // Create a new packet for each broadcast
        Ptr<Packet> packet = Create<Packet>();

        // Set the TTL tag, decrementing if necessary
        SocketIpTtlTag ttl;
        ttl.SetTtl(/* Set appropriate TTL value, e.g., */ 1); 
        packet->AddPacketTag(ttl);

        // Add the RREP header and TypeHeader to the packet
        packet->AddHeader(rrepHeader);
        packet->AddHeader(tHeader);

        // Determine the broadcast destination based on the interface's subnet mask
        Ipv4Address destination;
        if (iface.GetMask() == Ipv4Mask::GetOnes())
        {
            // Use all-hosts broadcast if the subnet mask is /32
            destination = Ipv4Address("255.255.255.255");
        }
        else
        {
            // Use subnet-directed broadcast otherwise
            destination = iface.GetBroadcast();
        }
        ...
        // Schedule the packet to be sent, optionally introducing a small random delay
        ...
        NS_LOG_INFO("Broadcasted RREP to " << destination);
    }
}
...
void
RoutingProtocol::RevRecvRequest(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
    NS_LOG_FUNCTION(this);
    RevRreqHeader rreqHeader;
    ...
    /*
     * When the reverse route is created or updated, perform the following actions:
     * 1. Compare and update the Originator Sequence Number.
     * 2. Set the valid sequence number field to true.
     * 3. Update the next hop in the routing table.
     * 4. Update the hop count.
     * 5. Set the Lifetime appropriately.
     */
    RoutingTableEntry toOrigin;
    ...
    NS_LOG_LOGIC(receiver << " received RREQ with hop count "
     ...
     * (ii) it has an active route to the destination with valid sequence number,
     *      and other conditions as per raodv protocol.
     */
    if (IsMyOwnAddress(rreqHeader.GetDst()))
    {
         ...
         * Check if the current node is the source. If yes, unicast the RREP; else, broadcast it.
         */
        bool isSource = IsMyOwnAddress(rreqHeader.GetOrigin());

        if (isSource)
        {
            /*
             * **Case 1: Current node is the source (originator) of the RREQ**
             * - Update the routing table accordingly.
             * - Unicast the RREP to the originator.
             */

            NS_LOG_DEBUG("Current node is the source. Sending RREP via unicast to originator.");

            // Assuming 'toOrigin' contains the necessary information to send the RREP
            // Create the RREP packet
             RevRreqHeader  rrepHeader(0,
                       0,
                       rreqHeader.GetHopCount(),
                       rreqHeader.GetId(),
            ...
            NS_LOG_INFO("Unicasted RREP to " << toOrigin.GetNextHop());
        }
        else
        {
            /*
             * **Case 2: Current node is NOT the source**
             * - Update the routing table accordingly.
             * - Broadcast the RREP to all relevant interfaces.
             */

            NS_LOG_DEBUG("Current node is not the source. Broadcasting RREP.");
            ...
            for (auto j = m_socketAddresses.begin(); j != m_socketAddresses.end(); ++j)
            {
                ...
                // Set the TTL tag, typically 1 for local broadcast
                SocketIpTtlTag ttlTag;
                //ttlTag.SetTtl(1); 
                ttlTag.SetTtl(tag.GetTtl()-1);
                packet->AddPacketTag(ttlTag);
    ...
    // Remove the TTL tag from the received packet
    SocketIpTtlTag tag;
    ...
    /*
     * **Original RREQ Broadcast Logic Starts Here**
     * The node rebroadcasts the RREQ if it has not already generated a RREP.
     */
    for (auto j = m_socketAddresses.begin(); j != m_socketAddresses.end(); ++j)
    {
        Ptr<Socket> socket = j->first;
    ...
    /*
     * **Original RREQ Broadcast Logic Ends Here**
     */
}
...
RoutingProtocol::RecvRequest(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
    ...
    if (IsMyOwnAddress(rreqHeader.GetDst()))
    {
        ...
        RevSendReply(rreqHeader, toOrigin);
        return;
    }
    /*
     * (ii) or it has an active route to the destination, the destination sequence number in the
